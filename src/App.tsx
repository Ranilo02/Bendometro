...
export default function App() {
  ...
  return (
    <div className={cn("app-shell", screenRotated && "app-shell--rotated")}>
      <main
        className="app-main mx-auto flex h-full min-h-full w-full max-w-[1280px] flex-col justify-center gap-4 px-4 py-4"
      >
        <header className="flex min-h-14 items-center justify-between gap-3 rounded-[1rem] border border-[var(--app-panel-border)] [background:var(--app-panel)] px-4 shadow-[inset_0_1px_0_var(--app-panel-highlight),0_10px_24px_rgba(0,0,0,0.25)]">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <button
              type="button"
              onClick={() => setScreenRotated((current) => !current)}
